...
public class AdaptiveStreamingController {
	...
	public ResponseEntity<UrlResource> getManifest(@PathVariable("id") String id) throws MalformedURLException {
		...
		UrlResource manifest = new UrlResource(String.format("file:movies/%s", optionalManifest.get().getUrl()));
			
		return ResponseEntity.status(HttpStatus.PARTIAL_CONTENT).contentType(MediaTypeFactory.getMediaType("application/dash+xml").orElse(MediaType.APPLICATION_OCTET_STREAM)).body(manifest);
	}
}
